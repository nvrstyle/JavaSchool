package ru.sbp.school.kafka.producer.test.model;
...
import static ru.sbp.school.kafka.producer.test.model.OperationType.*;
...
public class Transaction implements Partitionable, Identifiable {
    ...
    public Transaction(OperationType operationType, BigDecimal amount, String account, LocalDateTime date) {
        this(UUID.randomUUID(), operationType, amount, account, date);
    }

    public Transaction(UUID uuid, OperationType operationType, BigDecimal amount, String account, LocalDateTime date) {
        this.uuid = uuid;
